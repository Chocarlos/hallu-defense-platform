...
`ToolCallEnvelope` keeps its six required v1 fields, but `schema`, `risk_level`, `approval_required`, and `caller_context` are untrusted assertions. The API resolves `tool_name` through a server-side, versioned registry, selects distinct input/output schemas, and fails closed on unknown names or any metadata mismatch. Registry version/digest, policy action, and side effects remain private server metadata and are intentionally absent from the v1 wire contract.
For output validation, the sanitized value is checked against the trusted output schema a second time. Schema-compatible redaction markers can produce a rewrite; an incompatible marker produces a block with `sanitized_output: null`, and neither REST nor MCP receives the original value.
Approval IDs and execution tokens are not caller-asserted approval status. A token is single-use and is accepted only through an internal issuer-bound capability for the originating approval ID/trace, authenticated tenant and subject, tool/action, canonical argument hash, and current trusted definition version/digest.
`PolicyEvaluationRequest.attributes` can supply restrictive observations for simulation, but caller-provided approval, tenant identity, and deterministic-evidence flags are ignored. Python policy and Rego evaluate only a strictly typed, request/action/subject-bound server context with identical primary-rule precedence; `PolicyEvaluationResponse` is informational and is not an execution capability.
