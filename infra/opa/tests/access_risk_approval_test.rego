...
test_high_risk_with_approval_is_allowed {
	result := decision with input as {
		"action": "deploy",
		"risk_level": "critical",
		"attributes": {
...
test_secret_leakage_blocks_output {
	result := decision with input as {
		...
		"output": {
			"secret_findings": ["credential-like-value"],
		},
	}
...
test_secret_leakage_takes_precedence_over_pii_redaction {
	result := decision with input as {
		"action": "validate_tool_output",
		"risk_level": "medium",
		"attributes": {
...
test_sandbox_allowlisted_network_requires_approval {
	result := decision with input as {
		"action": "run_repo_checks",
...
test_repo_claim_with_deterministic_command_evidence_is_allowed {
	result := decision with input as {
		...
		"evidence": [
			{
				"kind": "command_output",
				"structured_content": {
					"metadata_schema": "sandbox_command.v1",
				},
			},
		],
	}
